            ...
            if decoded_header[NetIDEOps.NetIDE_header['TYPE']] is NetIDEOps.NetIDE_type['NETIDE_MODULE_ANN']:
                ...
                if decoded_header[NetIDEOps.NetIDE_header['MOD_ID']]==0:
                    if message_data == evenlope:   
                        ...
                        print "------------------a new backend and module-------------------"
                        get_the_id = self.distribute_id()  
                        self.controller.backend_list.append([get_the_id,str(evenlope)])
                        self.controller.backend_to_module.append([get_the_id])
                        
                        get_the_id = self.distribute_id()
                        self.controller.module_list.append([get_the_id, str(message_data)])
                        self.controller.backend_to_module[-1].append(get_the_id)
                        ...
                        print "backend_to_module",self.controller.backend_to_module,self.controller.backend_list,self.controller.module_list
...
#                         print "whuwzp:test"
    ...
    def send_to_clients(self, datapath, msg_buf, module_id ):
        ...
        print "self.backend_to_module",self.backend_to_module
