...
public class ZeroMQBaseConnector implements Runnable {
	...
	private ModuleHandlerImpl module_ann_test;
	...
	public boolean SendData(byte[] data) {
		...
		if (context != null) {
			...
			msg.send(sendSocket);
			System.out.println("--------message to send is " + "" + data);
			sendSocket.close();
			return true;
		}
			...
			if (poller.pollin(0)) {
				...
				if (coreListener != null) {
					...
					if (msg instanceof HelloMessage) {
						...
						System.out.println("********receive an openfolw message");
...
//						LOG.info("Module Announcement received. ModuleName: " + moduleName + " ModuleId: " + moduleId);
						...
						System.out.println("********Module Announcement received. ModuleName: " + moduleName + " ModuleId: " + moduleId);
//						module_ann_test.obtainModuleId_test(2, moduleName, moduleId);
					}
				}
			}
			if (poller.pollin(1)) {
				ZMsg message = ZMsg.recvMsg(controlSocket);
				System.out.println("~~~~~~~~receive message from control_socket" + message.toString());
